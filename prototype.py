...
def extract_mawb_from_bottom(pil_image):
    ...
    mawb = extract_mawb_number_simple(text2)
    if mawb:
        return mawb
...
def detect_airline_page(pil_image):
    ...
    for stage_name, func in stages:
        try:
            ...
            processed_img = (
                Image.fromarray(processed) if isinstance(processed, np.ndarray) else processed
            )
...
def extract_mawb_pages(pdf_path, output_dir):
    ...
    for i, page in enumerate(pages):
        ...
        # Try full page if still no number
        if not mawb:
        ...
        # Naming
        if mawb:
            out_file = os.path.join(output_dir, f"{mawb}.pdf")
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            out_file = os.path.join(output_dir, f"unknown_{filename}_p{page_num}_{timestamp}.pdf")
